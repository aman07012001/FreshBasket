...
import { FreshBasketServicesContext } from "../Home/OurServices/OurServices";
...
const OurServicesAndQualities = ({ children }) => {
    ...
    const isService = useContext(FreshBasketServicesContext);
                            ...
                            <div className={`${isDownloadApp ? 'xl:space-y-6 space-y-4' : 'md:space-y-2.5 sm:space-y-1.5  space-y-2.5'} w-11/12`}>
                                ...
                                {isDownloadApp ?
                                    <h1 className='pb-0 xl:text-3xl md:text-2xl text-xl font-bold capitalize xl:tracking-wide'>
                                        Download Our FreshBasket <br /> Mobile App And Save <br /> Time, Money
                                    </h1>
                                    : <h1 className='pb-0 md:text-2xl text-xl font-semibold capitalize xl:tracking-wide'>
                                        {isOurQuality ?
                                            'Best Quality Healthy And Fresh FreshBasket'
                                            : isService ?
                                                'Why We’re Best Quality FreshBasket Shopper '
                                                : ''}
                                    </h1>}
                                {}
                                <p className='lg:text-base text-gray-7o 00 md:text-sm sm:text-xs text-sm'>
                                    {isOurQuality ? 'We prioritize quality in each of our FreshBasket, below are the advantage of our products. Organic food is food produced.' : isService ? 'We offer a wide variety of products to choose from you can find everything you need to feed your family.' : 'Our FreshBasket mobile app makes it easy to shop for groceries on the go.You can browse our selection of products, create a shopping list, and order groceries for pickup or delivery.'}
                                </p>
                            </div>
